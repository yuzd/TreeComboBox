...
//			.append(elem.sub_info);
		...
		_getData: function(self, param) {
			...
			self.prop.xhr = $.getJSON(self.option.src, params, function(data) {
				if (data && data.length > 0) {
					...
					$.each(data, function(i, item) {
						var ret_ul_li = $('<li>');
						var li_label = $('<label>').addClass(self.css_class.collapsed).text(item.type).mouseup(function(event) {
						...
						$.each(item.list, function(i, item) {
							...
							var ret_li_ul_li = $('<li>').append(ret_li_ul_li_id).append(ret_li_ul_li_name).mouseover(function() {
		...
		_selectCurrentLine: function(self, target) {
			var current_val = $(target).find("div:first").text();
			...
			if (self.option.bind_to) {
				$(self.elem.combo_input).trigger(self.option.bind_to);
			}
			self.prop.has_select = true;
